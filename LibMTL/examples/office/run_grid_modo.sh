...
for lr in $LR_SET; do
    for gamma in $GAMMA_SET; do
        for rho in $RHO_SET; do
            ...
            python -u train_office.py --multi_input --seed $seed --gpu_id $gpu_id --dataset_path $dataset_path --weighting $weighting  --lr $lr --gamma_modo $gamma --rho_modo $rho  > hp_grid_logs/$weigting-seed-$seed-lr-$lr-gamma-$gamma-rho-$rho.out &
        done
    done
done