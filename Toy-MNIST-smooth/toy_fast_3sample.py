...
parser.add_argument('--num_samples_modo', type=int, default=3, help='number of gradient samples to use (2 or 3)')
...
modo_num_samples = params.num_samples_modo
...
# MoDo
modo_kwargs = {'lambd':torch.ones(num_tasks)/num_tasks, 'gamma':modo_gamma, 'rho':modo_rho, 'num_samples':modo_num_samples}
...
for i in range(num_epochs):
    ...
    if moo_method=='MoDo':
        ...
        for j in range(3):
            ...
            loss_list = [loss_list[k] + (1/3) * loss_list_[k] for k in range(num_tasks)]
