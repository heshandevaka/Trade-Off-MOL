...
from moo import grad_ew, grad_mgda, grad_moco, grad_modo
...
print(f'LOG FORMAT: Epoch: EPOCH | Train: {" ".join([f"LOSS{i+1}" for i in range(num_tasks)])}' 
      + f' | Test: {" ".join([f"LOSS{i+1}" for i in range(num_tasks)])}'
      + f' | Error: POP OPT GEN')
print("="*100)
for i in range(num_epochs):
    ...
    if i in T_list:
        # get loss and error values
        train_acc_, train_loss_, train_ps_ = get_performance(model, optimizer, train_eval_dataloader, loss_dict, num_param, num_param_layer, softmax, onehot_enc)
        test_acc_, test_loss_, test_ps_ = get_performance(model, optimizer, test_dataloader, loss_dict, num_param, num_param_layer, softmax, onehot_enc)
        ...
        # print loss values
        print(f"Epoch: {i: 8,} | Train: { ' '.join(str(round(num, 4)) for num in train_loss_) }"
               +f" | Test: { ' '.join(str(round(num, 4)) for num in test_loss_) }"
               +f" | Error: {round(test_ps_, 5)} {round(train_ps_, 5)} {round(test_ps_-train_ps_, 5)}")
    elif i%1000==0:
        print(f"Epoch: {i: 8,}")
...
# test_loss_ = get_loss(model, loss_dict, test_dataloader, softmax, onehot_enc)
# print loss values
print(f"Epoch: {i+1: 6,} | Train: { ' '.join(str(round(num, 4)) for num in train_loss_) }"
        +f" | Test: { ' '.join(str(round(num, 4)) for num in test_loss_) }"
        +f" | Error: {round(test_ps_, 5)} {round(train_ps_, 5)} {round(test_ps_-train_ps_, 5)}")
...
print(f'Script time: {time.time() -  t1}')
