...
def get_grad_vec(model, num_param, num_param_layer):
    ...
    grad_vec = torch.zeros(num_param).to(model.device)
...
def set_grads(model, multi_grad, num_param_layer):
    ...
    for param in model.parameters():
        ...
        if param.grad is not None:
            ...
            param.grad.data = multi_grad[beg:end].view(param.data.size()).data.clone().to(model.device)
        count += 1   
    return 
...
# use gpu id if available, else cpu
...
print(f"Model device check: {model.device}")
...
onehot_enc = torch.eye(10).to(model.device)
...
# MoCo
moco_kwargs = {'y':torch.zeros(num_tasks, num_param).to(device),
                'lambd':torch.ones([num_tasks, ]).to(device)/num_tasks, 
                'beta':moco_beta, 'gamma':moco_gamma, 'rho':moco_rho}

# MoDo
modo_kwargs = {'lambd':torch.ones(num_tasks).to(device)/num_tasks, 
               'gamma':modo_gamma, 'rho':modo_rho}
