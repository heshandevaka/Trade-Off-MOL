...
class Toy(nn.Module):
    ...
    def __forward__(self, x1, x2, z=[0,0], compute_grad=False): 
        ...
        f1 = f1 * c1 + (f1_sq - 1.8*z[0] * x1 + 5.* z[1] * x2) * c2
        f2 = f2 * c1 + (f2_sq + 1.8*z[0] * x1 + 5.* z[1] * x2) * c2 
...
def plot_contour(F, emp=True, task=1, traj=None, 
    ...
    if task == 0:
        ...
        plt.plot(-10, -10, marker='o', markersize=10, zorder=5, color='k')
    ...
    if plotbar:
        ...
        # cfbar = fig.colorbar(cf, ticks=None)
        # cfbar.set_ticks([])
        # cfbar.ax.tick_params(labelsize=tick_fontsize)
...
def plot_2d_pareto(method, out_path="", data_type='pop'):
    ...
    # if method=="modo":
    #     t1 = torch.load(f"backup/modo/toy0-runs1.pt")
    #     t2 = torch.load(f"backup/modo/toy1-runs1.pt")
    #     t3 = torch.load(f"backup/modo/toy2-runs1.pt")
    # else:
    ...
    for i, tt in enumerate(losses):
        ...
        colors = matplotlib.cm.plasma_r(np.linspace(0.6, 0.1, tt.shape[0]))
    ...
    # else:
        # ax.set_title(title_map[method], fontsize=25)
    plt.savefig(
        f"./{out_path}/{out_path}-{data_type}-pf.png",
        bbox_inches="tight",
        facecolor="white",
    )        
    return
...
def run_all(seeds=1, lr = 0.005, gamma_modo=0.08, out_path=""):
    ...
    for i, init in enumerate(inits):
                ...
                for it in range(n_iter):
                    ...
                    if m=="mgd":
                        ...
                        if it==0:
                            ...
                            # gamma = 0.08
                            rho = 1e-9
                        _, grads1 = F(x, True, 'pop', batch_size//2)
                        _, grads2 = F(x, True, 'pop', batch_size//2)
                        g, lambd = solver(grads1, grads2, lambd, gamma_modo, rho)
        ...
        torch.save(all_traj, f"./{out_path}/toy{i}-runs{seeds}.pt")
        torch.save(all_traj_error_norm, f"./{out_path}/toy{i}-error_norm-runs{seeds}.pt")
        torch.save(
            all_traj_true_dir_norm, f"./{out_path}/toy{i}-true_dir_norm-runs{seeds}.pt")
...
def plot_results(out_path=''):
    ...
    if not os.path.exists(f"./{out_path}/imgs"):
        os.makedirs(f"./{out_path}/imgs")

    plot_contour(F, task=1, levels=levels, name=f"./{out_path}/imgs/_toy_task_1")
    plot_contour(F, task=2, levels=levels, name=f"./{out_path}/imgs/_toy_task_2")
    ...
    for method in key_list: #
        ranges = list(range(10, length, 5000))
        ranges.append(length-1)
        for t in tqdm(ranges):
            # if method == "modo":
            plot_contour(F, task=0, levels=levels,
                            # task == 0 means plot for both tasks
                    ...
                    plotbar=(method == "modo"),
                    name=f"./{out_path}/imgs/toy_{method}_{t}")
...
if __name__ == "__main__":
    ...
    gamma=1.0
    lr=0.005
    out_path = f'modo_lr-{lr}_gamma-{gamma}_pop'
    if not os.path.exists(out_path):
        os.makedirs(out_path)
    
    # run_all(lr=lr, gamma_modo=gamma, out_path=out_path) 

    # Plot trajectories    
    plot_results(out_path=out_path)    

    # plot 3d PS measure surface
    # plot3d_PS(F)   

    ## Plot Pareto trajectory in Pareto front
    # plot_2d_pareto("modo", data_type='pop')
    # plot_2d_pareto("modo", data_type='pop', out_path=out_path)
    # plot_2d_pareto("mgd", data_type='pop')
    # plot_2d_pareto("mgd", data_type='emp')
    # plot_2d_pareto("sgd", data_type='pop')
    # plot_2d_pareto("sgd", data_type='emp')
    # plot_2d_pareto("smgd")
